...
struct SetupGuideView: View {
    ...
    private var steps: [SetupStep] {
        [
            SetupStep(
                id: "accessibility",
                title: "辅助功能",
                detail: model.axOK ? "已授权，可以代你按键" : "打开设置后，在列表里打开 FoloVibe Bridge",
            ...
            SetupStep(
                id: "bluetooth",
                title: "蓝牙",
                detail: model.bleSnap.bluetoothOn ? "蓝牙已开启" : "打开蓝牙，程序才能连接设备",
            ...
            SetupStep(
                id: "audio",
                title: "虚拟麦克风",
                detail: model.audioOK
                    ? "声音送进 \(model.settings.current.outputDevice)，语音软件里也选它"
                    : audioHint,
                symbol: "waveform",
                tint: .purple,
                ok: model.audioOK,
    ...
    /// Says what is actually available, so a first-time user is not told to
