...
`./build.sh` 会先跑 `swift run FoloVibeCoreTests` 再打包。不需要完整 Xcode，Swift 5.9+ 工具链和 Apple Command Line Tools 即可。给 FoloVibe Bridge 打开蓝牙和辅助功能。Typeless 听写快捷键与“说话”键一致（默认 Fn，也兼容 F13–F20，包括 F19），麦克风选 `BlackHole 2ch`。
