<p align="right">
  <strong>简体中文</strong> · <a href="doc-conventions.en.md">English</a>
</p>
...
- 所有维护中的 Markdown 文档，其默认 `name.md` 路径必须使用简体中文；英文使用配对的 `name.en.md`。
...
- 所有任务只强制先读仓库根 `AGENTS.en.md`。
- 按 `AGENTS.en.md` 的路由表读取与当前修改相关的文档和源码，不默认读取全部 README。
- 产品总览见 `docs/README.en.md`；需要发现其它文档时再读取 `docs/INDEX.en.md`。
...
- **fork 项目文档**：只描述某个 fork 的产品需求、业务逻辑或素材，放在 fork 自己的根 `README.en.md` 或 `docs/assets/`，不回合上游。

## 放置位置

- 仓库根目录仅保留工具发现入口（`AGENTS.en.md`、`CLAUDE.en.md` 及其翻译）、可选的 fork README 配对、许可证、构建清单和 ESP-IDF 配置。
...
- 仓库检查会拒绝新增其它根目录 Markdown。不要为了显眼而把文档放根目录，应从 `docs/INDEX.en.md` 建立入口。

不要创建只有目录说明而没有实际用途的空骨架。新增文档必须在 `docs/INDEX.en.md` 或所属目录索引中登记；删除或移动文档时同步更新所有链接。
...
- 用户可见行为、兼容性或发布流程变化记录到 `docs/CHANGELOG.md`；内部整理和拼写修复不强制记录。
