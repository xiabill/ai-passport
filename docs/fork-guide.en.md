<p align="right">
  <a href="fork-guide.md">简体中文</a> · <strong>English</strong>
</p>
...
The root `README.en.md` path is intentionally available to a fork owner. Upstream's project overview is `docs/README.en.md`, which GitHub displays when no root README exists. A fork may add its own root README to explain its product without replacing upstream documentation.

## Fork rules

- Keep fork `main` synchronized with `FoloToy/ai-passport:main`.
- On fork `main`, limit fork-owned content to a root `README.en.md` pair and `docs/assets/`; develop firmware and other changes on `feature/*` branches and merge by pull request.
