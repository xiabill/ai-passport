...
本仓库提供一套基于 GitHub Actions 的自动构建与发布流水线，用于在打 tag 时自动编译 macOS Bridge、固件并发布 Release。
...
2. **编译与验证固件**（ESP-IDF 5.5.3 / esp32c3）：运行与本地相同的 `./tools/validate.sh --firmware`。脚本使用 `sdkconfig.defaults` 和 `partitions.csv` 构建固件，再执行 `idf.py merge-bin`。
...
6. **构建 macOS Bridge**：在 macOS 14 runner 上运行 Bridge 测试和 `build.sh`，打包为 `FoloVibeBridge-macos.zip`，并与固件放入同一个 Release。
...
- `FoloVibeBridge-macos.zip`：macOS 伴侣程序，解压后将 `FoloVibeBridge.app` 拖到 `/Applications`。
