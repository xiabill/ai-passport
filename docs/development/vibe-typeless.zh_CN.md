...
- 标准模式：默认背光 50%，开始讲话 3 秒后降到 15%，空闲 18 秒也降到 15%，5 分钟关闭背光，连续空闲 15 分钟后深度睡眠；按确认发送时会短暂恢复到 50%。连接电脑 USB 后，自动降亮度、关屏和深度睡眠都会暂停。
- 省电模式：空闲 10 秒降到 8%，1 分钟关闭背光，连续空闲 5 分钟后深度睡眠；设备断开时 BLE 广播闲置 60 秒后暂停。省电模式同样适用“连接电脑后保持唤醒”的规则。
