...
- 标准模式：默认背光 50%，开始讲话 3 秒后降到 15%，空闲 18 秒也降到 15%，5 分钟关闭背光，连续空闲 15 分钟后深度睡眠；按确认发送时会短暂恢复到 50%。
- 省电模式：空闲 10 秒降到 8%，1 分钟关闭背光，连续空闲 5 分钟后深度睡眠；设备断开时 BLE 广播闲置 60 秒后暂停。
- 两种模式都使用 GPIO0 三键分压作为唤醒源。待机阶段第一次按键只唤醒屏幕；深睡唤醒会完整重新启动应用。省电模式下按普通功能键还会恢复 BLE 广播，便于 Mac 自动重连。
- 豆包上键：单击启动/停止豆包，快速双击执行 macOS 全选（Cmd+A），长按执行全选并删除，清空当前输入框内容。若在豆包录音中触发编辑操作，会先停止录音再执行。
- 音效：开始录音播放上扬双音，结束录音播放低沉长音，便于不看屏幕时确认状态。
- 两种模式都不是电池物理断电；如需零功耗，仍需使用硬件电源键。
