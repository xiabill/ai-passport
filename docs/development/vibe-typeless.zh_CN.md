...
- 使用中背光 100%，空闲 18 秒降到 20%，5 分钟后进入待机并关闭背光。
- 连续空闲 15 分钟后进入 ESP32-C3 深度睡眠，关闭主程序与 BLE；任意 GPIO0 普通功能键都可以唤醒并重新启动。
- 待机阶段的第一次功能键按下只负责唤醒屏幕；15 分钟后的深睡会进行一次完整启动，但不等同于电池物理断电。
