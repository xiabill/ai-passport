<p align="right">
  <a href="experience-notes.md">简体中文</a> · <strong>English</strong>
</p>
...
stays in the fork per [`docs/fork-guide.en.md`](../fork-guide.en.md).

Before starting a new development, check here for previously recorded, reusable
experience — alongside [`plays/`](../../plays/README.en.md) for reference
applications.
...
`.en.md` in English, aligned in the same change.

Each entry is a single `.md` file with its paired `.en.md`, stored under
`docs/experiences/<username>/` and named after the entry's content summary in
lowercase-kebab-case (for example `audio-compression-trade-offs.en.md`), so the
...
entries per developer**, each as its own file (with its paired `.en.md`) under
...
[`INDEX.en.md`](../experiences/INDEX.en.md) for the archived-entry table, for the
stored entries. The index below lists each entry once it is added.

- **Audio Compression Trade-offs on ESP32-C3** (Shinku-Chen) — how a voice-playback codec was chosen on limited flash (IMA-ADPCM vs Opus vs MP3), with measured capacity and decoder cost. See [`../experiences/shinku-chen/audio-compression-trade-offs.en.md`](../experiences/shinku-chen/audio-compression-trade-offs.en.md).
- **Post-Release Follow-up for the AI Passport Publishing Flow** (Shinku-Chen) — confirm the publish destination, include the data partition in a release, and the consent gates for the post-release tracks. See [`../experiences/shinku-chen/post-release-follow-up.en.md`](../experiences/shinku-chen/post-release-follow-up.en.md).
- **Display Refresh and Deep-sleep on ESP32-C3 (No PSRAM)** (Shinku-Chen) — direct panel refresh of a single image rect, RTC-GPIO deep-sleep wakeup, and the LVGL object-type misuse crash signature. See [`../experiences/shinku-chen/display-refresh-and-deep-sleep.en.md`](../experiences/shinku-chen/display-refresh-and-deep-sleep.en.md).
- **Network Audio Streaming and Memory Budgeting on AI Passport** (PhoenixZHC) — bounded HTTP audio streaming, ES8311/I2S ownership, and joint memory budgeting for decoding, JSON, DMA, and LVGL. See [`../experiences/phoenixzhc/network-audio-streaming-and-memory.en.md`](../experiences/phoenixzhc/network-audio-streaming-and-memory.en.md).
- **SoftAP Provisioning and Resource Budgets on AI Passport** (PhoenixZHC) — DHCP state, captive-portal compatibility, bounded forms and uploads, and no-PSRAM resource planning. See [`../experiences/phoenixzhc/softap-provisioning-and-resource-budget.en.md`](../experiences/phoenixzhc/softap-provisioning-and-resource-budget.en.md).
