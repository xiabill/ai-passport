...
- 先读 [`README.md`](../docs/README.md) 的硬件能力契约，以及
  [AI 硬件开发指南](../docs/hardware-design/AI_HARDWARE_DEVELOPMENT_GUIDE.md) 的完整硬件上下文。
- 参与社区时请遵守 [`CODE_OF_CONDUCT.en.md`](CODE_OF_CONDUCT.en.md)；普通使用问题见 [`SUPPORT.en.md`](SUPPORT.en.md)。
- 不要提交凭证、令牌、授权文件或个人数据。
- 仓库的 `main` 分支始终与上游基线保持同步；fork 用户在 `feature/*` 分支开发功能（见 `docs/fork-guide.en.md`）。

## 开发与验证

使用 ESP-IDF 5.5.3。全新机器先按
[环境引导](../docs/development/environment-setup.md)完成安装。
...
2. PR 标题使用 `<type>(<scope>): <简短描述>`，例如 `feat(bsp): ...`、`docs: ...`。
   可用 type 见 [`docs/contribution/commit-and-pr.md`](../docs/contribution/commit-and-pr.md)。
3. Review 完整 diff，确认没有凭证、无关生成文件或意外改动。
4. 遵守 `docs/contribution/commit-and-pr.en.md` 的 PR 要求：说明测试的硬件/版本、
...
[`SECURITY.en.md`](SECURITY.en.md) 的私密报告流程处理。
